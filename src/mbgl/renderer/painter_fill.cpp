...
void Painter::renderFill(FillBucket& bucket, const FillLayer& layer, const TileID& id, const mat4& matrix) {
    ...
    const bool pattern = !properties.pattern.from.empty();
    ...
    if (pattern) {
        ...
        if (pass == RenderPass::Translucent) {
            ...
            const SpriteAtlasPosition posA = spriteAtlas->getPosition(properties.pattern.from, true);
            const SpriteAtlasPosition posB = spriteAtlas->getPosition(properties.pattern.to, true);
            ...
            matrix::scale(patternMatrixA, patternMatrixA,
                    1.0f / (posA.size[0] * factor * properties.pattern.fromScale),
                    1.0f / (posA.size[1] * factor * properties.pattern.fromScale));
            mat3 patternMatrixB;
            matrix::identity(patternMatrixB);
            matrix::scale(patternMatrixB, patternMatrixB,
                    1.0f / (posB.size[0] * factor * properties.pattern.toScale),
                    1.0f / (posB.size[1] * factor * properties.pattern.toScale));
